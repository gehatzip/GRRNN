...
from model_utils import get_best_device, activ_func_from_type, reset_weights, optimizer_from_type, loss_for_model
from rcgan import plot_real_vs_gen

class SingleStepForecaster(nn.Module):

  def __init__(self):

    super(SingleStepForecaster, self).__init__()
  ...
  def train_cross_validation(self, X_folds, y_folds, config, verbose=True, extra_data = None):
    ...
    for valid_fld in fld_range:
      ...
      X_valid = torch.from_numpy(X_folds[valid_fld]).type(torch.FloatTensor).to(self.get_device())
      y_valid = torch.from_numpy(y_folds[valid_fld]).type(torch.FloatTensor).to(self.get_device())
      ...
      if extra_data != None:
        extra_data['predictions'][fold_offsets[valid_fld]+window_size] = y_valid_pred.detach().cpu().numpy()
  ...
  def train(self, X_train, y_train, config, verbose = True):
    ...
    for epoch in range(num_epochs):
      ...
      while batch_start < X_train.shape[0]:
        ...
        #forward feed
        y_pred_batch = self(X_train_batch)
...
class RNNForecaster(SingleStepForecaster):
  ...
  def __init__(self, input_size=1, output_size=1, hidden_size = 64, hidden_layers = 1):
    ...
    self.lstm = nn.LSTM(input_size = input_size, hidden_size = hidden_size, proj_size = output_size, num_layers = hidden_layers, device=self.device)

  def trainable_parameters(self):
    params = sum(p.numel() for p in self.lstm.parameters() if p.requires_grad)
    return params
  ...
  def forward(self, x):
    ...
    hidden = torch.zeros((self.num_layers, batch_size, self.output_size), device=self.device) # N_layers * batch_size * hidden_size (proj_size = 0 - if proj_size>0 it would be: N_layers * batch_size x proj_size)
    cell = torch.zeros((self.num_layers, batch_size, self.hidden_size), device=self.device) # N_layers * batch_size x hidden_size

    out = torch.zeros((batch_size, 1, self.output_size), device=self.device)

    seq_dim = 0 # 'time sequence dimension' = 0 (batch_first = False)

    for t in range(x.shape[1]):
        x_t = x[:,t,:].unsqueeze(seq_dim) 
        out, (hidden, cell) = self.lstm(x_t, (hidden, cell))
  
    return out.squeeze(seq_dim)

# Same as RNNForecaster but to forecast probabilities (output is softmaxed).
class RNNPROBForecaster(SingleStepForecaster):

  def __init__(self, input_size=1, output_size=1, hidden_size = 64, hidden_layers = 1):

    super(RNNPROBForecaster, self).__init__()

    self.rf = RNNForecaster(input_size, output_size, hidden_size, hidden_layers)


  def trainable_parameters(self):
    return self.rf.trainable_parameters()
...
class MLPForecaster(SingleStepForecaster):

  def __init__(self, input_size=1, output_size=1, hidden_size = 16, act_type = 'ReLU'):

    # input_size should be window_size*n_features

    super(MLPForecaster, self).__init__()
    
    self.fc = nn.Linear(input_size, hidden_size, device=self.device)
    self.hidden = nn.Linear(hidden_size, output_size, device=self.device)
    self.act = activ_func_from_type(act_type)

  def trainable_parameters(self):
    params = sum(p.numel() for p in self.fc.parameters() if p.requires_grad)
    params += sum(p.numel() for p in self.hidden.parameters() if p.requires_grad)
    return params


  def forward(self, x_win_feat):

    # x_win_feat: batch_size x input_window x n_features        
    batch_size = x_win_feat.shape[0]

    x = x_win_feat.view(batch_size, x_win_feat.shape[1]*x_win_feat.shape[2])
